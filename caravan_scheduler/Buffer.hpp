...
public:
  ...
  void Run(const std::vector<int>& consumers) {
    ...
    while(true) {
      logger.d("%d tasks, %d running_tasks, %d requesting_workers, %d task_resutls, %d consumers, request_sent %d", tasks.size(), running_task_ids.size(), requesting_workers.size(), task_results.size(), consumers.size(), request_sent );
